...
const Home = ({ user }: any) => {
  ...
  useEffect(() => {
    if (cars) {
      ...
      const unfinishedCars = cars?.cars?.filter((car) =>
  ...
  console.log("unpaied repairs ==>", unpaiedRepairs);
    ...
    if (unpaiedRepairs.length > 0) {
      ...
      for (let index = 0; index < 3; index++) {
        ...
        if (unpaiedRepair) {
          ...
          tempResults.push(<><Box className={classes.awaitingWrapper}>
            <Typography fontSize={16} fontWeight={theme.typography.fontWeightBold}>{owner}</Typography>
            <Typography fontSize={16} fontWeight={theme.typography.fontWeightBold}>{carNumber}</Typography>
            <Typography fontSize={16} fontWeight={theme.typography.fontWeightBold}>{formatDate(day)}</Typography>
          </Box>
            <Divider />
          </>
          )
      ...
      <Grid item xs={12} container>
        <Grid item xs={4} sx={{ paddingTop: "15px", paddingLeft: "15px", paddingRight: "15px" }}>
          <Box className={classes.boxWrapper}>
            ...
            <Typography variant="h6" color={todaysProfit > 0 ? "#50BC1D" : theme.palette.text.primary} fontWeight={theme.typography.fontWeightBold} fontSize="32px" sx={{ paddingLeft: "20px", paddingTop: "20px" }}>
        ...
        <Grid item xs={4} sx={{ paddingTop: "15px", paddingLeft: "15px", paddingRight: "15px" }}>
          <Box className={classes.boxWrapper}>
            ...
            <Typography variant="h6" color={weeklyProfit > 0 ? "#50BC1D" : theme.palette.text.primary} fontWeight={theme.typography.fontWeightBold} fontSize="32px" sx={{ paddingLeft: "20px", paddingTop: "20px" }}>
        ...
        <Grid item xs={4} sx={{ paddingTop: "15px", paddingLeft: "15px", paddingRight: "15px" }}>
          <Box className={classes.boxWrapper}>
            ...
            <Typography variant="h6" color={monthlyProfit > 0 ? "#50BC1D" : theme.palette.text.primary} fontWeight={theme.typography.fontWeightBold} fontSize="32px" sx={{ paddingLeft: "20px", paddingTop: "20px" }}>
          ...
          <Grid item xs={8} sx={{ paddingLeft: "15px", paddingRight: "15px", }}>
            <Box className={classes.boxWrapper} style={{ zIndex: '-2', paddingTop: "10px", }}>
              <ResponsiveContainer width="100%" height="100%" minHeight="245px">
                <LineChart data={dataForLiveData?.combinedData}
            ...
            <Box className={classes.boxWrapper} style={{ zIndex: '-2', marginTop: "30px" }}>
              <Typography fontSize="22px" fontWeight={theme.typography.fontWeightBold} style={{ padding: "26px" }} >Чакащи плащания</Typography>
              <Box style={{ backgroundColor: theme.palette.primary.light, padding: "14px", paddingRight: "10%", display: "flex", justifyContent: "space-between" }}>
                <Typography fontSize={16} fontWeight={theme.typography.fontWeightBold}>Име на клиент</Typography>
                <Typography fontSize={16} fontWeight={theme.typography.fontWeightBold}>Номер на колата</Typography>
                <Typography fontSize={16} fontWeight={theme.typography.fontWeightBold}>Дата на изпълнение</Typography>
              </Box>
...
export const getServerSideProps = wrapper.getServerSideProps((state) => async (context: GetServerSidePropsContext) => {
  return {
    props: {},
  };
});